...
func (s *BlockChainAPI) needToReplay(ctx context.Context, block *types.Block, accounts []common.Address) (bool, error) {
	...
	parentState, err := s.b.Chain().StateAt(parent.Root(), parent.Hash(), parent.Number())
	if err != nil {
		return false, fmt.Errorf("statedb not found for block number (%d): %v", block.NumberU64()-1, err)
	}
	currentState, err := s.b.Chain().StateAt(block.Root(), block.Hash(), block.Number())
	if err != nil {
...
func (s *BlockChainAPI) replay(ctx context.Context, block *types.Block, accounts []common.Address) (*types.DiffAccountsInBlock, *state.StateDB, error) {
	...
	statedb, err := s.b.Chain().StateAt(parent.Root(), parent.Hash(), block.Number())
	if err != nil {
...
func SubmitTransaction(ctx context.Context, b Backend, tx *types.Transaction) (common.Hash, error) {
	...
	if tx.To() == nil {
		...
		log.Debug("Submitted contract creation", "hash", tx.Hash().Hex(), "from", from, "nonce", tx.Nonce(), "contract", addr.Hex(), "value", tx.Value(), "x-forward-ip", xForward)
	} else {
		log.Debug("Submitted transaction", "hash", tx.Hash().Hex(), "from", from, "nonce", tx.Nonce(), "recipient", tx.To(), "value", tx.Value(), "x-forward-ip", xForward)
	}
	return tx.Hash(), nil
}
